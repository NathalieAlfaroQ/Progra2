...
int Jugadores::jugar(int modo, int dificultad) {
  ...
  while (!victoria && !empate) {
    ...
    do {
      ...
      if (modo == 1) {
        cout << "Jugador " << jugador << ", ingresa el número de columna (0-"
      ...
      // Persona vs robot.
        ...
        if (jugador == 1) {
          ...
          cout << "\n La IA ha jugado en la columna: " << columna << endl;
        }  // End if
      }
      // Robot 1 vs robot 2.
      else if (modo == 3) {
        /*Crea un número aleatorio entre 0 y la cantidad de columnas menos uno
        si no se puede hacer una jugada decisiva.*/
        if (dificultad == 2) {
          columna = tablero.victoriaIA();
        } else {
          columna = -1;
